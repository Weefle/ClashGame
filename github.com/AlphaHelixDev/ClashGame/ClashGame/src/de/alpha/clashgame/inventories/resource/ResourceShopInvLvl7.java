...
public class ResourceShopInvLvl7 {
		...
		if(RatHausManager.getRatHausLvL(p) == 7){
			...
			{	if(ElexirlagerManager.getAmount(p) <= 1) {
					...
					inv.setItem(10, new ItemCreator(Material.DIAMOND_BLOCK).setName("§8Elexirlager").setLore(new String[]{
							"", "§cDu hast schon 6 ElexirLager!"
					}).build());
				}
			}
			
			{	if(ElexirSammlerManager.getAmount(p) <= 5) {
			...
			{	if(GoldMineManager.getAmount(p) <= 5) {
			...
			{	if(GoldLagerManager.getAmount(p) <= 1) {
